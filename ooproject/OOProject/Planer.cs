...
class Planer
    {
        ...
        public Planer(string sciezka)
        {
            ...
            foreach(string linia in linie)
            {
                ...
                switch(dane[0])
                {
                    case "AKTYWNOSC":
                        ...
                        listaWydarzen.Add(
                            new Aktywnosc(
                                dane[1],
                                dane[2],
                                DateTime.Parse(dane[3]),
                                DateTime.Parse(dane[4]),
                                dane[5]
                            ));
                        break;
                    ...
                    case "AUTOBUS":
                        ...
                        listaWydarzen.Add(
                            new Autobus(
                                DateTime.Parse(dane[1]),
                                DateTime.Parse(dane[2]),
                                dane[3],
                                dane[4],
                                dane[5],
                                dane[6],
            ...
            {
                dataPoczatek =
                    listaWydarzen[0].GetDataPoczatek();
            ...
            try
            {
                using(StreamWriter sw = new StreamWriter(sciezka))
                {
                    foreach(Wydarzenie w in listaWydarzen)
                    {
                        ...
                        else if(w is Autobus ab)
                        {
                            sw.WriteLine(
                                ...
                                $"{ab.GetNotatka()};" +
                                $"{ab.GetMiejsce()};" +
                                $"{ab.GetPrzystanekStartowy()};" +
                                $"{ab.GetPrzystanekKoncowy()};" +
                                $"{ab.GetLiczbaPrzystankow()}");
                        }
                    ...
                    foreach(Wydarzenie n in listaNoclegi)
                    {
                        ...
                        sw.WriteLine(
                            $"NOCLEG;" +
                            $"{nocleg.GetDataPoczatek()};" +
                            $"{nocleg.GetDataKoniec()};" +
                            $"{nocleg.GetNazwa()};" +
                            $"{nocleg.GetLokalizacja()};" +
                            $"{nocleg.GetTyp()};" +
                            $"{nocleg.GetNotatka()}");
                    }
                }
        ...
        public void WstawNocleg(Wydarzenie nocleg)
        {
            try
            {
                ...
                foreach (Wydarzenie n in listaNoclegi)
                {
                    if (nocleg.GetDataPoczatek() < n.GetDataKoniec() &&
                        nocleg.GetDataKoniec() > n.GetDataPoczatek())
                    {
                        kolizja = true;
                        break;
                    }
                ...
                while (indeks < listaNoclegi.Count &&
                    listaNoclegi[indeks].GetDataPoczatek() <
                    nocleg.GetDataPoczatek())
                {
                    indeks++;
                }
        ...
        private List<int> SprawdzTerminyNoclegow(TimeSpan time)
        {
            ...
            {
                TimeSpan czasPomiedzy =
                    listaNoclegi[i + 1].GetDataPoczatek() -
                    listaNoclegi[i].GetDataKoniec();
        ...
        public void PrzestawNocleg(int index)
        {
            ...
            try
            {
                ...
                List<int> mozliweMiejsca =
                    SprawdzTerminyNoclegow(nocleg.GetCzasTrwania());
                ...
                {
                    nowyStart =
                        listaNoclegi[nowyIndex - 1]
                        .GetDataKoniec();
                }

                DateTime nowyKoniec =
                    nowyStart + nocleg.GetCzasTrwania();
            ...
            catch (Exception ex)
            {
                ...
                {
                    if (index <= listaNoclegi.Count)
                        listaNoclegi.Insert(index, nocleg);
                    else
                        listaNoclegi.Add(nocleg);
                }
            }
        }
        public void PrzestawWydarzenie(int index)
        {
            ...
            try
            {
                ...
                List<int> mozliweMiejsca =
                    SprawdzTerminy(wydarzenie.GetCzasTrwania());
                ...
                {
                    nowyStart =
                        listaWydarzen[nowyIndex - 1]
                        .GetDataKoniec();
                }

                DateTime nowyKoniec =
                    nowyStart + wydarzenie.GetCzasTrwania();

                wydarzenie.UstawTermin(
                    nowyStart,
                    nowyKoniec);

                listaWydarzen.Insert(
                    nowyIndex,
                    wydarzenie);

                Console.WriteLine(
                    "Przestawiono wydarzenie.");
            ...
            Wydarzenie nowyNocleg)
        {
            ...
            {
                if(nowyNocleg.GetDataPoczatek() < nocleg.GetDataKoniec() &&
                nowyNocleg.GetDataKoniec() > nocleg.GetDataPoczatek())
                {
                    return false;
                }
            }
        ...
        public bool ZmienCzasTrwaniaNoclegu(
            int index,
            TimeSpan nowyCzasTrwania)
        {
            if(index < 0 || index >= listaNoclegi.Count)
                return false;
            ...
            DateTime staryKoniec =
                nocleg.GetDataKoniec();

            DateTime nowyKoniec =
                nocleg.GetDataPoczatek() + nowyCzasTrwania;
            ...
            bool poprawnyTermin =
                SprawdzTerminNoclegu(nocleg);

            if(poprawnyTermin)
            {
                nocleg.ZmienCzasTrwania(
                    nowyCzasTrwania);
        ...
        public bool ZmienCzasTrwaniaWydarzenia(
            int index,
            TimeSpan nowyCzasTrwania)
        {
            if(index < 0 || index >= listaWydarzen.Count)
            ...
            DateTime staryKoniec =
                wydarzenie.GetDataKoniec();

            DateTime nowyKoniec =
                wydarzenie.GetDataPoczatek() + nowyCzasTrwania;
            listaWydarzen.RemoveAt(index);
            ...
            bool poprawnyTermin =
                SprawdzTermin(wydarzenie);

            if(poprawnyTermin)
            {
                wydarzenie.ZmienCzasTrwania(
                    nowyCzasTrwania);
